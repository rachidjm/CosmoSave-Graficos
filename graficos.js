...
// Insertar gráfico y escalar proporcional
async function insertChartAndFit({ presId, slideId, chartId, pgW, pgH }) {
  ...
  // 3. Escalar proporcional para que no se corte el mapa y funcionen tooltips
        ...
        requests: [
          {
            updatePageElementTransform: {
              objectId: chartElemId,
              applyMode: 'ABSOLUTE',
              transform: { scaleX, scaleY, shearX: 0, shearY: 0, translateX: 0, translateY: 0, unit: 'PT' }
            }
          }
        ]
      }
...
async function main() {
  ...
  if (!token) {
    console.error('❌ No se pudo obtener access token OAuth');
    process.exit(1);
  }
  ...
  for (const [tienda, { sheetName, folderId }] of Object.entries(TIENDAS)) {
    ...
    if (!sh) {
      console.log(`⚠️ Hoja no encontrada: "${sheetName}" (${tienda})`);
      continue;
    }
    const charts = sh.charts || [];
    if (!charts.length) {
      console.log(`ℹ️ ${tienda} / ${sheetName}: sin gráficos incrustados`);
      continue;
    }
